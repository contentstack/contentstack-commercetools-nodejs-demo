...
if (env === 'default') _env = require(path.join(_path, 'default'));
else _env = require(path.join(_path, env));

// load globals

global['config'] = _env;
global['Stack'] = Contentstack.Stack(config.contentstack.api_key, config.contentstack.delivery_token, config.contentstack.environment);
